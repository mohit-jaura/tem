...
protocol ViewableControllerDelegate: AnyObject {
...
protocol ViewableControllerDataSource: AnyObject {
...
class ViewableController: UIViewController {
    ...
    lazy var pauseButton: UIButton = {
        let button = UIButton(type: .custom)
        let image = UIImage(named: "pauseWhite")!
        button.setImage(image, for: UIControl.State())
        button.alpha = 0
    ...
    var videoCurrentDuration:Double = 0.0
    ...
    override func viewDidLoad() {
        ...
        //To check device is in background mode or in foreground mode.....
...
//                        self.imageView.image = image
                        if let urlString = url,
                            ...
                            self.imageView.kf.setImage(with: url, placeholder:#imageLiteral(resourceName: "ImagePlaceHolder"))
                        }
                        self.configure()
                    }
                }
            ...
            let shouldAutoplayVideo = self.dataSource?.viewableControllerShouldAutoplayVideo(self) ?? false
                ...
                self.videoView.prepare(using: viewable) {
                    if shouldAutoplayVideo {
                        ...
                        if (self.videoCurrentDuration > 0.0) {
                            self.videoView.play(duration: self.videoCurrentDuration)
                            self.videoCurrentDuration = 0.0
                        } else {
                           self.videoView.play()
                        }
