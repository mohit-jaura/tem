...
class VideoView: UIView {
    ...
    private var shouldRegisterForFailureOrEndingNotifications = true
    private var shouldRegisterForOutputVolume = true

    private var playbackProgressTimeObserver: Any?
...
//        try? AVAudioSession.sharedInstance().setActive(true)
    ...
    func play(duration:Double? = 0.0) {
        ...
        // add delay to wait palyer status....
        
        if (duration! > 0.0){
            DispatchQueue.main.asyncAfter(deadline: .now() + 5.0) {
                self.stopPlayingAndSeekSmoothlyToTime(duration: duration!)
                self.playerLayer.player?.play()
    ...
    func stopPlayingAndSeekSmoothlyToTime(duration: Double) {
       // guard let timescale = self.playerLayer.player?.currentItem?.currentTime().timescale
        guard let timescale = self.playerLayer.player?.currentItem?.asset.duration.timescale else { return }
...
extension VideoView {
    ...
    private func addPlayer(using viewable: Viewable, completion: @escaping () -> Void) {
    ...
    private func removeBeforePlayingObservers() {
    ...
    private func removeWhilePlayingObservers() {
        if let playbackProgressTimeObserver = self.playbackProgressTimeObserver {
