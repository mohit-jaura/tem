...
public class YPLibraryVC: UIViewController, YPPermissionCheckable {
    ...
    private func checkVideoLengthAndCrop(for asset: PHAsset,
        ...
        if fitsVideoLengthLimits(asset: asset) == true {
            delegate?.libraryViewStartedLoading()
            var normalizedCropRect : CGRect = DispatchQueue.main.sync { v.currentCropRect() }
            if let cropRect = withCropRect {
                ...
                for asset in selectedAssets.enumerated() {
                    ...
                    case .image:
                        self.fetchImageAndCrop(for: asset.element.asset, withCropRect: asset.element.cropRect) { image, exifMeta in
                            let photo = YPMediaPhoto(image: image.resizedImageIfNeeded(), exifMeta: exifMeta, asset: asset.element.asset)
                    ...
                    case .video:
                        ...
                        self.checkVideoLengthAndCrop(for: asset.element.asset, withCropRect: asset.element.cropRect) { videoURL in
