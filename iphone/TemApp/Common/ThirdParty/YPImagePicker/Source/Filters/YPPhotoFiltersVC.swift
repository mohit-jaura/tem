...
open class YPPhotoFiltersVC: UIViewController, IsMediaFilterVC, UIGestureRecognizerDelegate {
    ...
    private let filters: [YPFilter] = YPConfig.filters

    private var selectedFilter: YPFilter?
    
    private var filteredThumbnailImagesArray: [UIImage] = []
    private var thumbnailImageForFiltering: CIImage? // Small image for creating filters thumbnails
    private var currentlySelectedImageThumbnail: UIImage? // Used for comparing with original image when tapped

    private var v = YPFiltersView()
