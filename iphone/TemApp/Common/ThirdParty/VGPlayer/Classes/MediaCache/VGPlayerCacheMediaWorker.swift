...
open class VGPlayerCacheMediaWorker: NSObject {
    open private(set) var cacheConfiguration: VGPlayerCacheMediaConfiguration?
    ...
    private let writeFileQueue = DispatchQueue(label: "com.vgplayer.cacheWriteFileQueue")
    ...
    open func cache(_ data: Data, forRange range: NSRange, closure: (Bool) -> Void) {
        writeFileQueue.sync {
            ...
            if writeFileHandle?.seek(toFileOffset: UInt64(range.location)) != nil, writeFileHandle?.write(data) != nil {
    ...
    open func set(cacheMedia: VGPlayerCacheMedia) -> Bool {
        ...
        if writeFileHandle?.truncateFile(atOffset: UInt64(cacheMedia.contentLength)) != nil, writeFileHandle?.synchronizeFile() != nil {
            return true
        } else {
            return false
        }
