...
open class VGPlayerDownloadActionWorker: NSObject {
    ...
    open private(set) var actions = [VGPlayerCacheAction]()
    open private(set) var url: URL
    open private(set) var cacheMediaWorker: VGPlayerCacheMediaWorker
    
    open private(set) var session: URLSession?
    open private(set) var task: URLSessionDataTask?
    open private(set) var downloadURLSessionManager: VGPlayerDownloadURLSessionManager?
    open private(set) var startOffset: Int = 0
