...
open class VGPlayerCacheMediaConfiguration:NSObject, NSCoding, NSCopying {
    ...
    private let cacheSegmentQueue = DispatchQueue(label: "com.vgplayer.CacheSegmentQueue")
    private let cacheDownloadInfoQueue = DispatchQueue(label: "com.vgplayer.CacheDownloadInfoQueue")
...
extension VGPlayerCacheMediaConfiguration {
    public func save() {
        self.cacheSegmentQueue.sync {
             _ = NSKeyedArchiver.archiveRootObject(self, toFile: filePath!)
        }
    }
    
    
    public func addCache(_ segment: NSRange) {
        ...
        self.cacheSegmentQueue.sync {
            ...
            if count == 0 {
                ...
                if indexSet.count > 1 {
                    ...
                    _ = indexSet.sorted(by: >).map {cacheSegments.remove(at: $0)}
    ...
    public func add(_ downloadedBytes: UInt64, time: TimeInterval) {
