...
open class VGPlayerView: UIView {
    ...
    open private(set) var placeholderImageView: UIImageView?
    open private(set) var playerLayer : AVPlayerLayer?
    open private(set) var isFullScreen : Bool = false
    open private(set) var isTimeSliding : Bool = false
    open private(set) var isDisplayControl : Bool = true {
        didSet {
    ...
    open var soundButton : UIButton = UIButton(type: .custom)
    ...
    open private(set) var panGestureDirection : VGPlayerViewPanGestureDirection = .horizontal
    ...
    //correspond to the tag of display view with respect to the section and the row
    ...
    public override init(frame: CGRect) {
        ...
        configurationUI()
//        #imageLiteral(resourceName: "yp_iconFlash_on")
    }
    ...
    public convenience init(viewFrame: CGRect) {
    ...
    open override func layoutSubviews() {
        ...
        if let previewUrl = previewUrl {
            self.placeholderImageView?.kf.setImage(with: previewUrl, placeholder: #imageLiteral(resourceName: "ImagePlaceHolder"))
        }
        updateDisplayerView(frame: bounds)
    }
    
    open func setPreviewWith(url: String) {
    ...
    open func reloadPlayerLayer() {
        ...
        //layer.insertSublayer(self.playerLayer!, at: 0) //previous
        layer.insertSublayer(self.playerLayer!, at: 1) //new
...
//        configurationBottomView()
        ...
        configurationLoadingIndicatorView()
//        setupViewAutoLayout()
    }
    
    open func displayUIControls() {
    ...
    open func reloadPlayerView() {
        ...
        if let placeholderView = self.placeholderImageView {
...
extension VGPlayerView {
    ...
    @objc open func onSingleTapGesture(_ gesture: UITapGestureRecognizer) {
        ...
        self.delegate?.didTapOnVGPlayerView(self)
    }
    ...
    internal func panGestureHorizontal(_ velocityX: CGFloat) -> TimeInterval {
        ...
        if vgPlayer?.currentDuration != nil, let totalDuration = vgPlayer?.totalDuration {
...
//        fullscreenButton.isHidden = true
    ...
    internal func configurationLoadingIndicatorView() {
...
//            make.bottom.equalTo(strongSelf)
        ...
        playButtion.snp.makeConstraints { [weak self] (make) in
            guard let strongSelf = self else { return }
            make.left.equalTo(strongSelf.bottomView).offset(15)
