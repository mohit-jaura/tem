...
import AVKit
import Kingfisher
protocol MediaImageCollectionViewCellDelegate: AnyObject {
...
class MediaImageCollectionViewCell: UICollectionViewCell {
    ...
    // MARK: Properties
    weak var delegate: MediaImageCollectionViewCellDelegate?
    var player: AVPlayer?
    ...
    func initialize(atIndexPath indexPath: IndexPath, withMediaItem media: Media) {
        if let type = media.type,
            ...
            switch type {
            case .photo:
                self.imageView.kf.setImage(with: url, placeholder: #imageLiteral(resourceName: "ImagePlaceHolder"), options: nil, progressBlock: { (receivedSize, totalSize) in
...
extension MediaImageCollectionViewCell: UIScrollViewDelegate {
...
extension MediaImageCollectionViewCell: UIGestureRecognizerDelegate {
