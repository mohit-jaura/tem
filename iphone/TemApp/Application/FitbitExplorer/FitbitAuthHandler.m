...
-(void)login:(UIViewController*)viewController {
  ...
  //  FitBitLogin *obj = [[FitBitLogin alloc] init];
    [Utility presentFitBitControllerWithUrl:[NSString stringWithFormat:@"%@?response_type=code&client_id=%@&redirect_uri=%@&scope=%@&expires_in=%@",authUrl,clientID,redirectURI,defaultScope,expiresIn]];
...
-(void)getAccessToken:(BOOL)success {
     if (success) {
        ...
        manager.responseSerializer.acceptableContentTypes = [manager.responseSerializer.acceptableContentTypes setByAddingObject:@"application/x-www-form-urlencoded"];
         ...
         [manager POST:@"https://api.fitbit.com/oauth2/token" parameters:param headers: nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
             //******************** Save Token to NSUserDedaults ******************
             [Utility removeFitBitController];
...
-(void)revokeAccessToken:(NSString *)token {
    ...
    [manager POST:@"https://api.fitbit.com/oauth2/revoke" parameters:params headers: nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
