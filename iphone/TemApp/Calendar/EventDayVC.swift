...
//  TemApp
//
//  Created by PrabSharan on 01/07/22.
...
class EventDayVC: DIBaseController, KVKCalendarSettings {
    ...
    func createCalendarStyle() -> Style {
        ...
        style.headerScroll.colorBackground = .newAppThemeColor
        style.headerScroll.titleDateColor = .white
        style.headerScroll.colorBackgroundCurrentDate = .appThemeColor
        style.headerScroll.colorBackgroundSelectDate = .appThemeColor
        style.headerScroll.colorWeekendDate = .gray
        style.headerScroll.colorDate = .white
        style.headerScroll.colorNameDay = .white
        style.headerScroll.titleDateFont = .boldSystemFont(ofSize: 16)
        style.headerScroll.heightHeaderWeek = 60
        style.headerScroll.heightSubviewHeader = 72
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let calendar = KVKCalendarView(frame: CGRect(x: 0, y: 0, width: calendarUIView.frame.width, height: calendarUIView.frame.height), style: style)
...
//            }
    ...
    private func monthChangedUsingArrows(_ action:GoToCalendar?) {
        guard let selectedDate = selectedDate, let action = action else {return}
        switch action {
            case .NextMonth:
...
extension EventDayVC {
    private func addNotificationObserver() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(monthChangedNotification), name: NSNotification.Name(rawValue:Constant.NotiName.monthChanged), object: nil)
    }
    ...
    @objc func monthChangedNotification(_ not:NSNotification) {
        if let data = not.userInfo as NSDictionary?, let action = data["action"] as? GoToCalendar {
            monthChangedUsingArrows(action)
        }
    }
}

extension EventDayVC: CalendarDataSource {
    func eventsForCalendar(systemEvents: [EKEvent]) -> [Event] {
        return events
    }
}

extension EventDayVC: CalendarDelegate {
