...
public final class TimelineView: UIView {
    ...
    private lazy var allDayView: AllDayView = {
        ...
        allDayView.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 5).isActive = true
        allDayView.trailingAnchor.constraint(equalTo: trailingAnchor, constant: 5).isActive = true
    ...
    override public func draw(_ rect: CGRect) {
        ...
        if isToday {
            ...
            let minute = component(component: .minute, from: time)
            let hour = component(component: .hour, from: time)
    ...
    private func layoutNowLine() {
        if !isToday {
            ...
            let nowDate = currentTime.UTCToLocalString(inFormat: .preDefined).toDate(dateFormat: .preDefined)
            bringSubviewToFront(nowLine)
            nowLine.alpha = 1
            let size = CGSize(width: bounds.size.width, height: 20)
            let rect = CGRect(origin: CGPoint.zero, size: size)
            nowLine.date = nowDate
            nowLine.frame = rect
            nowLine.center.y = dateToY(nowDate)
        }
    }
