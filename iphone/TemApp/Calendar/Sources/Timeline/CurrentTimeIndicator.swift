...
@objc public final class CurrentTimeIndicator: UIView {
  ...
  @objc private func timerDidFire(_ sender: Timer) {
      date = Date().UTCToLocalDate(inFormat: .preDefined)
  }
  ...
  private func configureTimer() {
    invalidateTimer()
    let currentDate = Date().UTCToLocalDate(inFormat: .utcDate)
    var components = calendar.dateComponents(Set([.era, .year, .month, .day, .hour, .minute]), from: currentDate)
    components.minute! += 1
